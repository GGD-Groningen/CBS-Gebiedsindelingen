...
#   - Provincie-gemeente indeling: https://www.cbs.nl/nl-nl/onze-diensten/methoden/classificaties/overig/gemeentelijke-indelingen-per-jaar/indeling-per-jaar/gemeentelijke-indeling-op-1-januari-2021
#   - Buurt, wijk, gemeente: https://www.cbs.nl/nl-nl/maatwerk/2021/36/buurt-wijk-en-gemeente-2021-voor-postcode-huisnummer
...
#     - Province-municipality classification: https://www.cbs.nl/nl-nl/onze-diensten/methoden/classificaties/overig/gemeentelijke-indelingen-per-jaar/indeling-per-jaar/gemeentelijke-indeling-op-1-januari-2021
#     - Neighborhood, district, municipality: https://www.cbs.nl/nl-nl/maatwerk/2021/36/buurt-wijk-en-gemeente-2021-voor-postcode-huisnummer
...
pc_data <- read_delim(paste0("pc6hnr20210801_gwb.csv"), 
                     delim = ";", 
                     locale = locale(encoding = "UTF-8"))

# Read provincie data (Excel version)
provincie_data <- read_excel(paste0("Gemeenten-alfabetisch-2021.xlsx"), sheet = "Gemeenten_alfabetisch_2021")
...
buurt_data <- buurt_data %>% 
  select(-gm2021, -gm_2021, -gm_naam, -wk_2021, -wk2021, -wk_naam)
...
buurt_data <- buurt_data %>% 
  rename(!!paste0("buurt", year) := buurtcode2021)

wijk_data <- wijk_data %>% 
  rename(!!paste0("wijk", year) := wijkcode2021)

gemeente_data <- gemeente_data %>% 
  rename(!!paste0("gemeente", year) := gemcode2021)
